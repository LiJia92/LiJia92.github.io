...
tags:
 - AI
---
我们之前做的项目是关于科二考试的，里面涉及到非常多的规则、扣分设置。比如项目完成时间 210 秒，那如果超过这个时间没有完成项目就会进行扣分。类似的规则设置非常非常多，现状是这些规则都需要手动去设置，比较麻烦。而且只有对规则非常熟悉的人，才能找到对应的规则进行设置。于是我们想能不能用 AI 来帮助我们设置这些规则，用户输入自然语言，我们识别到具体的规则标识出来，并设置好具体的规则值。比如用户输入：“项目完成时间 210 秒”，我们就识别到项目完成时间这个规则，并且设置好规则值 210。当然也可以提供编辑框，对 210 进行编辑。鉴于我们的规则非常多，所以分两步来做：
1. 先识别具体的项目。若有多个，则弹窗由用户选择。
2. 将这个项目的所有规则，设置为本地数据，AI 根据这个本地数据给出修改建议。

我们现在的初步思路只验证规则设置，扣分的设置后面再举一反三。

<!-- more -->

通过 CodeBuddy IDE 结合 Claude-4.0-Sonnet，我做出了这样一个 demo：
<video controls="controls" width="100%" height="auto" name="Video Name" src="https://images-1258496336.cos.ap-chengdu.myqcloud.com/2025/8/demo.mp4"></video>

看起来效果还行吧，全程都是通过 CodeBuddy IDE 自行编写代码，体验下来效果还是不错的。中间过程有若干的错误、异常等等，也进行了非常多的调优，但是只要问题够精准，AI 就能实现得八九不离十。
![](https://images-1258496336.cos.ap-chengdu.myqcloud.com/2025/8/12.png)
...
![](https://images-1258496336.cos.ap-chengdu.myqcloud.com/2025/8/11.png)
...
```
代码还是很清爽的，命名、格式、大小写、缩进都非常讲究，给 CodeBuddy IDE 点赞👍！
在早期的 AI 探索中，面对一个问题，可能提示词不够精准，就很难用好 AI。**我们可以通过提示词，来限定 AI 返回的结果，这样就能让 AI 更加精准，就有点像请求我们自己的 API 接口一样了。**拿到结果直接解析，然后做后续的业务。但是前提是，提示词要足够精准才行。在 AI 的浪潮中，提示词才是我们必修的一门功课啊～